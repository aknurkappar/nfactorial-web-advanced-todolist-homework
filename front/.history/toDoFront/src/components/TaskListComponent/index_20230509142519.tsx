    ...
    <div>
      {
        taskList && taskList.map((task, index) => (
            ...
            <p className={`${item.isDone ? "done" : "todo-text"}`}>{task.title}</p>

            {nowActiveSection.name != "trash"  && task.isTrashModalShown && 
            <div className="trash-modal">
                <button onClick= {()=>{moveToTrash(task)}}>Move to Trash</button>
            </div>
            } 
